...
var DiagramNode,
    ...
    findHotPointBestMatch = function(diagramNode1, diagramNode2) {
        ...
        if (diagramNode1 == diagramNode2){
            var hp = diagramNode1.hotPoints;
            var length = hp.length;
            var bestMatch = [hp[Math.round(length/4-1)], hp[0]];
            return bestMatch;
        }
...
DiagramNode = A.Component.create({
    ...
    prototype: {
         ...
         * @param transition
         */
        alignTransition: function(transition, node) {
            var instance = this;
            ...
            if (diagramNode) {
                ...
                if (node != null){
                    console.log("xy");
                    A.mix(transition, {
                        sourceXY: node.sourceXY,
                        targetXY: node.targetXY
                    });
                }
         ...
         * @param optConnector
         */
        connect: function(transition, optConnector, node) {
            var instance = this;
                        ...
                        A.merge({
                            after: {
                                selectedChange: function() {
                                    instance.alignTransition(transition, node);
                                }
                            },
                            builder: builder,
        ...
        connectNode: function(diagramNode) {
            ...
            if (dd.startXY.length == 0){
                console.log("changed");
                var nodes = instance.get('builder').toJSON().nodes;
                dd.startXY = instance.get('builder').connector.get('p1');
                var p2 = instance.get('builder').connector.get('p2');
                if (p2.length == 0){
                    var xy = nodes[nodes.length - 1].xy;
                    dd.mouseXY = [xy[0], xy[1] + 10];
                }
                else{
                    dd.mouseXY = p2;
                }
            ...
            var t = instance.prepareTransition({
        ...
        prepareTransition: function(val) {
            ...
            if (A.Lang.isObject(val)) {
                target = val.target;
                sourceXY = val.sourceXY;
                targetXY = val.targetXY;
            }
            ...
            A.Array.some(
                ...
                function(t) {
                    transition = (target === t.target && sourceXY === t.sourceXY && targetXY === t.targetXY) ? t : null;
