...
var DiagramBuilder = A.Component.create({
    ...
    prototype: {
         ...
         * @param optConnector
         */
        connect: function(diagramNode1, diagramNode2, optConnector, node) {
            ...
            if (diagramNode1 && diagramNode2) {
                diagramNode1.connect(diagramNode2.get('name'), optConnector, node);
            }
        ...
        connectAll: function(nodes) {
            ...
            AArray.each(nodes, function(node) {
                if (node.hasOwnProperty('source') && node.hasOwnProperty('target')) {
                    instance.connect(node.source, node.target, node.connector, node);
                }
            });
