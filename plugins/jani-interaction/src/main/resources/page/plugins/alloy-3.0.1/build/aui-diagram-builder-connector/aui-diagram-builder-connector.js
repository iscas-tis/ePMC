...
A.Connector = A.Base.create('line', A.Base, [], {
    ...
    draw: function() {
        ...
        var s = instance._state.data.transition.lazy.value;

        var cp;

        if(s.source != null && s.source == s.target){
            ...
            var ratio = Math.sqrt(ds*ds + dt*dt);

            var os = constant * dt / ratio;
            var ot = constant * ds / ratio;

            console.log(os);
            console.log(ot);

            a1 = Math.pow((s1+s2)/2 + os - 35, 2) + Math.pow((t1+t2)/2 + ot - 35, 2);
            a2 = Math.pow((s1+s2)/2 + os - 35, 2) + Math.pow((t1+t2)/2 - ot - 35, 2);
            a3 = Math.pow((s1+s2)/2 - os - 35, 2) + Math.pow((t1+t2)/2 + ot - 35, 2);
            a4 = Math.pow((s1+s2)/2 - os - 35, 2) + Math.pow((t1+t2)/2 - ot - 35, 2);

            if (a1 >= a2 && a1 >= a3 && a1 >= a4){
                cp = [x1 + os, y1 + ot, x2 + os, y2 + ot, x2, y2];
            }
