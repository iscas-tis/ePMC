...
public final class LowLevelBioBuilder implements LowLevel.Builder {
    public final static String IDENTIFIER = "bio";
    ...
    public LowLevel build() {
        if (!(model instanceof ModelBio)) {
            return null;
        }
        if (engine instanceof EngineExplicit) {
            return ((ModelBio) model).newGraphExplicit(graphProperties, nodeProperties, edgeProperties);
        } else {
            return null;
        }
    }
