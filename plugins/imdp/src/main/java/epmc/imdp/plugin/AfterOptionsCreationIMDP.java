...
import epmc.imdp.graphsolver.GraphSolverExplicitIMDPJavaDouble;
...
public final class AfterOptionsCreationIMDP implements AfterOptionsCreation {
	...
	private void prepareLumpOptions(Options options) {
        ...
        Map<String,Method> cacheTypeMap = new OrderedMap<>(true);
        try {
        	cacheTypeMap.put("trove-hash", ProblemSetSolver.class.getMethod("newTroveHash"));
        	cacheTypeMap.put("java-util-tree", ProblemSetSolver.class.getMethod("newJavaUtilTree"));
        	cacheTypeMap.put("java-util-hash", ProblemSetSolver.class.getMethod("newJavaUtilHash"));
		} catch (NoSuchMethodException | SecurityException e) {
			throw new RuntimeException(e);
		}
        OptionTypeMap<Method> optionTypeCacheType = new OptionTypeMap<>(cacheTypeMap);
        options.addOption().setBundleName(OptionsIMDPLump.OPTIONS_IMDP_LUMP)
	...
	private void prepareGraphsolverOptions(Options options) {
        ...
        graphSolverMap.put(GraphSolverExplicitIMDPJavaDouble.IDENTIFIER, GraphSolverExplicitIMDPJavaDouble.class);
