...
public final class Term implements Comparable<Term>, Cloneable {
	...
	public Term(TypeFunctionPolynomial type, BigInteger coefficient, int[] exponents) {
		...
		if (exponents == null) {
			exponents = new int[type.getNumParameters()];
		}
	...
	public Term(TypeFunctionPolynomial type) {
		this(type, null, null);
	}
	...
	public static void toPolynomial(ValueFunctionPolynomial result,
			List<Term> terms) {
		assert result != null;
		assert terms != null;
