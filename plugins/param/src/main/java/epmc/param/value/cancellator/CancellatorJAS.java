...
public final class CancellatorJAS implements Cancellator {
	...
	public CancellatorJAS(TypeFunctionPolynomial type) {
		assert type != null;
		this.type = type;
		rebuildJASIfNeeded();
	}
	...
	private void rebuildJASIfNeeded() {
		if (previousNumParameters == type.getNumParameters()) {
			return;
		}
		BigInteger fac = new BigInteger();
		String[] pNameReversed = new String[type.getNumParameters()];
		for (int paramNr = 0; paramNr < pNameReversed.length; paramNr++) {
			pNameReversed[paramNr] = type.getParameters().get(paramNr).toString();
		}
		...
		this.previousNumParameters = type.getNumParameters();
	}

	@Override
	public void cancel(ValueFunctionPolynomial operand1,
			ValueFunctionPolynomial operand2) {
		rebuildJASIfNeeded();
	...
	private GenPolynomial<BigInteger> toJAS(ValueFunctionPolynomial operand) {
		assert operand != null;
	...
	private void toPolynomial(ValueFunctionPolynomial result,
