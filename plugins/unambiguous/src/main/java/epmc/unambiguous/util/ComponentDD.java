...
public class ComponentDD implements Closeable {
    ...
    public ComponentDD(GraphDD graph, DD nodes, boolean onlyBSCC, boolean skipTransient) throws EPMCException {
        assert graph != null;
        assert nodes != null;
        ...
        OptionsTypesEPMC.DdSccAlgorithm sccAlgorithm = options.getEnum(OptionsEPMC.DD_SCC_ALGORITHM);
        this.chatterjee = sccAlgorithm == OptionsTypesEPMC.DdSccAlgorithm.CHATTERJEE;
        if (isNondet) {
            startMECs(nodes);
        } else {
            startSCCs(nodes);
    ...
    public ComponentDD(GraphDD graph, DD nodes, boolean skipTransient) throws EPMCException {
        this(graph, nodes, true, skipTransient);
    }
    
    public DD next(DD notNodes) throws EPMCException {
        assert !closed;
        if (isNondet) {
            return nextMEC(notNodes);
        } else {
            return nextSCC(notNodes);
        }
    }
    ...
    private DD nextSCC(DD notNodes) throws EPMCException {
        DD found = null;
        while (found == null && !stack.isEmpty()) {
            ...
            switch (entry.where) {
            case FIRST: {
            	nodes = entry.nodes.clone().andNotWith(notNodes.clone());
            	restrictTrans(nextToPres, edges, nodes);
                if (nodes.isFalse()) {
                    entry.close();
                    continue;
                }
    ...
    private DD nextMEC(DD notNodes) throws EPMCException {
        while (true) {
            DD orig = nextSCC(notNodes);
            if (orig == null) {
