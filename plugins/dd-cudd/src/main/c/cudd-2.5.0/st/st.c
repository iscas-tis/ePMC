...
#define ST_NUMHASH(x,size) ((ptruint)(x)%(size))
...
#define ST_PTRHASH(x,size) ((unsigned int)((ptruint)(x)>>st_shift)%size)
...
st_delete_int(st_table *table, void *keyp, int *value)
{
    ...
    *last = ptr->next;
    if (value != NIL(int)) *value = (int) (ptrint) ptr->record;
    *(char **)keyp = ptr->key;
    FREE(ptr);
    table->num_entries--;
    return 1;
...
st_gen_int(st_generator *gen, void *key_p, int *value_p)
{
    ...
    *(char **)key_p = gen->entry->key;
    if (value_p != NIL(int)) {
   	*value_p = (int) (ptrint) gen->entry->record;
    }
    gen->entry = gen->entry->next;
    return 1;
