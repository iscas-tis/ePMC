 ...
 * @author Yong Li
 */
public final class PropertySolverExplicitLTLRA extends PropertySolverExplicitLTL {
	public final static String IDENTIFIER = "ltl-ra-explicit";
        ...
        if(! this.mecComputation) {
			...
			for (BitSet leafSCC = endComponents.next(); leafSCC != null; leafSCC = endComponents
				...
				for (AcceptanceCondition label : labels) {
					...
					AcceptanceLabel fin = label.getFiniteStates();
					if (fin == null || fin.isFalse())
						finStates = new BitSetUnboundedLongArray(leafSCC.size());
					else if (fin.isTrue())
