...
public final class ModuleCommands implements Module {
     ...
     */
    public ModuleCommands(ContextValue contextValue,
            String name,
        ...
        if (invariants == null) {
        	invariants = ExpressionLiteral.getTrue(contextValue);
        }
    ...
    ModuleCommands rename(String name, Map<Expression, Expression> map)
        ...
        for (Entry<Expression,JANIType> entry : this.variables.entrySet()) {
            ensure(map.containsKey(entry.getKey()), ProblemsGuardedCommand.VAR_NOT_RENAMED,
                    entry.getKey(), name);
...
//            entry.getValue().checkExpressionConsistency(types);
            ...
            if (init != null) {
                Type initType = init.getType();
                        ...
                        ProblemsGuardedCommand.VAR_INIT_INCONSISTENT, entry.getKey(), init);
            }
            */
        }
        for (Command command : commands) {
