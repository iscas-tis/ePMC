...
public final class OperatorEvaluatorMin implements OperatorEvaluator {
	@Override
	public boolean canApply(String operator, Type... types) {
		...
		if (!operator.equals(OperatorMin.IDENTIFIER)) {
			return false;
		}
		if (types.length != 2) {
			return false;
    ...
    public Type resultType(String operator, Type... types) {
    	assert operator != null;
    	assert operator.equals(OperatorMin.IDENTIFIER);
    	assert types != null;
    	for (Type type : types) {
    		assert type != null;
    	}
