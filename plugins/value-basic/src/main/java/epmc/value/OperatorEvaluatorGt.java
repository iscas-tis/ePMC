...
public final class OperatorEvaluatorGt implements OperatorEvaluator {
	@Override
	public boolean canApply(String operator, Type... types) {
		...
		if (!operator.equals(OperatorGt.IDENTIFIER)) {
			return false;
		}
		if (types.length != 2) {
			return false;
    ...
    public Type resultType(String operator, Type... types) {
    	assert operator != null;
    	assert operator.equals(OperatorGt.IDENTIFIER);
    	assert types != null;
    	for (Type type : types) {
    		assert type != null;
    	}
    ...
    public void apply(Value result, String operator, Value... operands) throws EPMCException {
    	...
    	ValueBoolean.asBoolean(result).set(ValueAlgebra.asAlgebra(operands[0]).isGt(operands[1]));
    }
}
