...
public final class OperatorEvaluatorGe implements OperatorEvaluator {
	@Override
	public boolean canApply(String operator, Type... types) {
		...
		if (!operator.equals(OperatorGe.IDENTIFIER)) {
			return false;
		}
		if (types.length != 2) {
			return false;
    ...
    public Type resultType(String operator, Type... types) {
    	assert operator != null;
    	assert operator.equals(OperatorGe.IDENTIFIER);
    	assert types != null;
    	for (Type type : types) {
    		assert type != null;
    	}
    ...
    public void apply(Value result, String operator, Value... operands) throws EPMCException {
    	...
    	ValueBoolean.asBoolean(result).set(ValueAlgebra.asAlgebra(operands[0]).isGe(operands[1]));
    }
}
