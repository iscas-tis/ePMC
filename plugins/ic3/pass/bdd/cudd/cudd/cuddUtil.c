...
static int ddCountMintermGMPAux (DdNode *node, mpz_t max, DdHashTable *table, mpz_t result);  /* modification: use bignum instead of double here */
  ...
  int  nvars)
{
    ...
    double	res;
    if(nvars < 1000) {
	    // Fabio's original faster code for fewer variables
	    double	max;
	    DdHashTable	*table;
	    ...
	    res = ddCountMintermGMPAux(node,max,table,result);
	    cuddHashTableQuit(table);
	    Cudd_SetEpsilon(manager,epsilon);
	    res = mpz_get_ui(result);
    }
...
*/
    ...
    if(ddCountMintermGMPAux(Nt,max,table,minT)==CUDD_OUT_OF_MEM)
	return CUDD_OUT_OF_MEM;
    if(ddCountMintermGMPAux(Ne,max,table,minE)==CUDD_OUT_OF_MEM)
    	return CUDD_OUT_OF_MEM;
    mpz_t divisor;
    mpz_init(divisor);
    mpz_set_ui(divisor,2);

    mpz_add(result,minT,minE);
    mpz_divexact(result,result,divisor);
